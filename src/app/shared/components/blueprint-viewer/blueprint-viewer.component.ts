...
import { ComputedConnector, ExtendedBlueprintNode, colocarNodos, trazarConectores } from './services/blueprint-router';
import { ThemeService } from '../../../core/services/theme.service';

// Los tipos viven con el router; se reexportan para no mover los imports.
export type { ComputedConnector, ExtendedBlueprintNode } from './services/blueprint-router';
...
export class BlueprintViewerComponent implements OnDestroy {
  ...
  // El trazado vive en services/blueprint-router.ts, como funciones puras:
  // asi se puede probar sin montar el componente (e2e/blueprint-diagramas.spec.ts).
  private readonly servicios = {
    layout: this.layoutService, positioning: this.positioningService, path: this.pathCalculator
  };

  readonly layoutNodes = computed(() =>
    colocarNodos(this.nodes() ?? [], this.edges() ?? [], this.servicios));

  readonly computedConnectors = computed<ComputedConnector[]>(() =>
    trazarConectores(this.edges() ?? [], this.layoutNodes(), this.servicios));
