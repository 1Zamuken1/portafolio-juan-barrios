...
- **Genera prosa, listas de texto y el diagrama.** No toca `techStack`, `structuredStack`, `structuredFeatures` ni `rawMetrics`. Las claves de esos campos ya son inconsistentes entre los cuatro proyectos —conviven `Arquitectura` y `Architecture`, `IA` y `Artificial Intelligence`, y tres variantes de «Formatos de exportación»— y un modelo generándolas libremente añadiría una cuarta cada vez. Los iconos son peor: son clases devicon concretas, que no se adivinan, solo se aciertan por casualidad. Los diagramas sí, pero partidos (ver abajo).
