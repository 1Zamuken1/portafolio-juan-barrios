...
- **Vidrio en el portafolio público: donde se interactúa mucho y se lee poco.** Los tokens `--pv-*` están en `styles/vidrio-publico.css`, en la raíz y con variante clara (Solarized). El cromo del editor (título, barra de actividad, explorador, pestañas y barra de estado) va en `vscode-layout.vidrio.css`, que se carga después del CSS del layout. El vidrio se nota por unas luces difusas fijas en el fondo del contenedor. `.editor-content` se queda **opaco**, porque es donde se lee. Lo que flota (tooltips, panel de perfil, explorador en móvil) y la cabecera de la ficha, que está sobre la imagen, llevan desenfoque real. Las tarjetas sobre el fondo plano lo simulan con `--pv-superficie` y un filo de luz, porque no tienen nada que desenfocar. De Material se toman las capas de estado (hover 6 %, pulsar 12 %), la elevación `--pv-elev-1..3` y las píldoras, no los componentes. **El caso de estudio, la trayectoria y las líneas del readme no se tocan.** Con `prefers-reduced-transparency` todo pasa a opaco y sin luces, y con `prefers-reduced-motion` se quitan los desplazamientos.
...
Ambos reciben `{ name, readme }` y devuelven `name`, `shortDescription`, `fullDescription`, las cinco secciones de `readmeMarkdown`, los `challenges` y, **opcionales**, las listas (`features`, `highlights`, `keywords`) y los tres resúmenes de arquitectura (`coreArchitecture`, `databaseArchitecture`, `aiArchitecture`).

**Listas y arquitectura: vacías antes que inventadas.** El prompt las pide solo si el readme las dice; si no, `[]` o `""`, nunca «N/A». Por eso el caso de uso no les pone mínimo, solo techo (10 entradas, 160 letras por entrada, 40 por palabra clave, 100 por resumen), y el formulario **solo escribe las que llegan con algo**: una vacía quiere decir «el readme no lo dice», y borrar con eso lo que ya hubiera sería perder un dato por no tener otro. La vista previa, con el borrador terminado, dibuja lo vacío como «El readme no lo dice» y no como hueco, que seguiría prometiendo algo que no va a llegar.
...
- **Genera prosa, listas, el stack por capas, las características por grupos y el diagrama.** No toca `techStack` ni `rawMetrics`. Las métricas son cifras, y es donde más fácil se inventa. `techStack` lleva clases de icono, que no se adivinan.
- **`structuredStack` y `structuredFeatures` siguen un vocabulario cerrado**, en `src/assets/data/vocabulario.json`:
  - **Stack:** nueve capas fijas (`backend`, `frontend`, `desktop`, `database`, `authentication`, `ai`, `automation`, `export`, `quality`), con su rótulo en español para la ficha.
  - **Características:** nueve grupos comunes en español (Arquitectura, Seguridad, IA…). Los grupos del dominio (Finanzas, Catálogo) son libres.
  - **Qué hace el backend:** `domain/service/Vocabulario` normaliza los sinónimos que escriba el modelo (`persistence` → `database`, `Architecture` → `Arquitectura`) sin gastar un reintento. Solo rechaza una capa que no es ninguna.
  - **Sin iconos:** el stack no lleva clases de icono; la ficha las saca del nombre de cada tecnología.
  - **Por qué:** antes cada proyecto usaba sus claves (`Arquitectura` y `Architecture`, tres «Formatos de exportación») y un modelo generándolas habría añadido una cuarta cada vez.
  - **Pruebas:** `VocabularioTest` compara la copia en Java con el JSON, y `e2e/vocabulario.spec.ts` obliga a los datos a cumplirlo.
- **Los diagramas, partidos (ver abajo).**
- **El diagrama: el modelo dice qué piezas hay, el código dice dónde van.** El modelo propone `architectureNodes` (id, label, description, group, type, icon) y `architectureEdges` (from, to), **sin coordenadas**. `domain/service/MaquetadorDeDiagrama` los coloca como en los diagramas hechos a mano: una columna por capa (client → application/automation → persistence/external → database), 380 px entre columnas y 160 entre filas, cajas de 250 × 96, columnas vacías cerradas, **cada caja a la altura media de las cajas con las que se une** (una pasada de izquierda a derecha y otra de vuelta: una cadena ORM → base de datos sale recta y quien reparte a tres queda en medio) y puertos según la dirección de la arista. Un modelo que escribe coordenadas las escribe distintas cada vez, y a menudo unas encima de otras. El caso de uso rechaza (y reintenta) un diagrama que no se puede dibujar: menos de 3 o más de 12 nodos, ids repetidos, una capa que el visor no conoce o una arista a un nodo que no existe. Vacío vale: el readme no da piezas.
- **Iconos solo de una lista cerrada** (`MaquetadorDeDiagrama.ICONOS`). La fuente de devicon del sitio está recortada a los glifos que usa, y una clase fuera de ella no da error: no se dibuja. El prompt da la lista, y un icono que no está se cambia por el de su capa. `MaquetadorDeDiagramaTest` comprueba contra `public/fonts/iconos.css` que cada devicon de la lista existe: si se regenera el recorte y alguno se cae, falla ahí.
- **Lo pendiente no está hecho.** Regla 5 del prompt: un módulo que el readme marca como pendiente no va en features, highlights, desafíos ni diagrama, aunque su tecnología esté en la tabla del stack. Con GastuApp las dos fichas, la hecha a mano y la de la IA, daban por integrado un agente que el readme lista como `# Pendiente`.
- **El diagrama no se salta capas que el readme nombra** (una capa de servicios va entre las vistas y el ORM), un módulo con sección propia es un nodo, un nodo por motor de datos, y los devicon solo para la pieza que *es* esa tecnología. Salen de comparar el primer diagrama de GastuApp con el hecho a mano.
- **El enlace a GitHub solo si está escrito en el readme.** El caso de uso compara la URL propuesta con el texto; si no aparece, la tira sin reintentar. Una URL bien formada puede ser igual de inventada.
- **En el formulario, el diagrama no se edita**: se ve en la sección Arquitectura y, si lo propuso la IA, se puede descartar. Solo viaja al guardar si es una propuesta de esa visita; si no, no se envía y el backend conserva el que había.
...
Las claves de `etapa` son estables porque la interfaz decide por ellas: `entrada`, `modelo`, `texto`, `respuesta`, `parseo`, `reintento`, `validacion`, `fin`, `error`.

**El plazo de la respuesta está fijado** (`spring.mvc.async.request-timeout=180s`). Sin él, Spring usa el del contenedor, unos 30 s; una redacción con reintento pasa de eso, y al vencer el plazo Spring intentaba pintar una página de error sobre una respuesta que ya había empezado a salir.
...
**La estación «Servidor» existe porque Render duerme.** A los quince minutos sin uso se apaga y tarda hasta dos en despertar, y esa espera caía entera en la burbuja del readme («137,8 s» por leer un texto que tarda cero). Ahora el primer aviso del backend (`entrada`) cierra el servidor —que se queda con la espera— y el readme a la vez. Además, el redactor hace un `GET /api/health` nada más abrirse, para que Render vaya despertando mientras se pega el readme, y un aviso en la cabecera dice si está despierto.

**Un borrador que no pasa las cotas, o una respuesta que no se puede leer como borrador, se pide otra vez, una sola.** El caso de uso le pasa al redactor el motivo tal cual lo dio la validación («llegaron 0 challenges»), que es lo que el modelo sabe arreglar; el puerto tiene para eso una variante de `draft` con `correccion`. Solo una porque cada intento es una llamada de pago: si falla dos veces, el problema está en la entrada. «Ilegible» es `RespuestaIlegibleException` (dominio): vino vacía, se cortó razonando (`finish_reason: length`), Groq metió un `error` en el flujo (`json_validate_failed`) o no tenía la forma esperada. Hereda de `DrafterNoDisponibleException`, así que lo que no la reintenta la trata como siempre; un 401, un 429 o un corte de red no la lanzan, porque saldrían igual la segunda vez. Los `gpt-oss` van con `reasoning_effort: low`: razonan antes de responder, y con el esfuerzo por defecto un readme largo se les iba entero en razonar. El panel lo ve como la etapa `reintento`, que en el reproductor abre un tramo nuevo: el texto del segundo borrador no se pega al del primero.
...
- **`publish` regenera `public/sitemap.xml`** y lo incluye en el commit. El primer proyecto publicado desde el panel entró sin él, y `e2e/sitemap.spec.ts` lo detectó después.
